...
function ChartsSkeleton() {
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-3" data-testid="analytics-charts-skeleton">
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className="rounded-xl border border-dash-border bg-dash-surface p-4 space-y-3">
          <Skeleton className="h-4 w-40 bg-dash-border" />
          <Skeleton className="h-52 w-full bg-dash-surface-muted" />
        </div>
      ))}
    </div>
  );
...
function SectionCard({ title, subtitle, children, testId }) {
  return (
    ...
    >
      <div className="mb-4">
        <h2 className="font-cabinet text-base md:text-lg font-bold text-dash-text tracking-tight">{title}</h2>
...
function PipelineButton({ label, count, amount, onClick, testId }) {
  return (
    <button
      ...
      onClick={onClick}
      data-testid={testId}
      className="w-full text-left rounded-lg border border-dash-border px-3 py-2.5 hover:border-dash-primary/30 hover:bg-dash-surface-muted transition-colors"
    >
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs text-dash-text-muted">{label}</span>
        <span className="text-sm font-semibold tabular-nums text-dash-text">{count}</span>
      </div>
      {amount != null ? (
        <p className="mt-1 text-[11px] tabular-nums text-dash-text-muted">{amount}</p>
      ) : null}
    </button>
  );
}
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6 md:space-y-8" data-testid="analytics-page">
      ...
      <p className="text-sm text-dash-text-muted leading-relaxed" data-testid="analytics-data-source-hint">
        {t("analyticsPage.dataSourceHint")}
      </p>

      <div className="flex flex-col gap-3">
        <div
          className="inline-flex flex-wrap items-center gap-1 rounded-xl border border-dash-border bg-dash-surface p-1 w-fit"
          role="tablist"
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          <button
            type="button"
            onClick={() => refetch()}
            className="text-sm font-semibold text-dash-primary hover:underline"
          >
        ...
        <>
          ...
          >
            {KPI_KEYS.map((key) => {
              ...
              return (
                <div
                  key={key}
                  className="rounded-xl border border-dash-border bg-dash-surface p-4 md:p-5 shadow-[0_1px_2px_rgba(10,37,64,0.04)] min-h-[108px]"
                ...
                >
                  ...
                  <p className="mt-2.5 font-cabinet text-xl md:text-2xl font-bold tracking-tight text-dash-text tabular-nums leading-none">
                    {noDecision ? "—" : formatKpiValue(kpi, lang)}
                  </p>
                  <div className="mt-3 min-h-[22px]">
                    {noDecision ? (
                      <span className="text-xs text-dash-text-subtle">{t("analyticsPage.kpi.noDecision")}</span>
                    ) : change ? (
                      <span
                        className={[
                      ...
                      >
                        {change}
                        <span className="ml-1 font-medium text-dash-text-muted">{t("analyticsPage.kpi.vsPrevious")}</span>
                      </span>
                    ) : (
                      <span className="text-xs text-dash-text-subtle">{t("analyticsPage.kpi.noComparison")}</span>
                    )}
                  </div>
                </div>
              );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-4">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 text-xs text-dash-text-muted border-t border-dash-border-soft pt-3">
                <div>
                  <p>{t("analyticsPage.quotes.proposed")}</p>
                  <p className="mt-0.5 text-sm font-semibold text-dash-text tabular-nums">
          ...
          >
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      active
                        ? "bg-[var(--dash-nav-active-bg)] text-white border-dash-primary"
                        : "bg-dash-surface text-dash-text-muted border-dash-border hover:text-dash-text",
                    ].join(" ")}
            ...
            {!topClients.length ? (
              <p className="text-sm text-dash-text-muted py-4">{t("analyticsPage.topClients.empty")}</p>
            ) : (
                  ...
                  <tbody>
                    {topClients.map((client) => {
                      ...
                      return (
                        <tr
                          key={client.clientId}
                          className="border-b border-dash-border-soft last:border-0 hover:bg-dash-surface-muted"
                        >
                          ...
                          <td className="py-2.5 px-2 text-right text-[13px] tabular-nums text-dash-text-muted hidden sm:table-cell whitespace-nowrap">
          ...
          {comparisonItems.length > 0 ? (
            <section
              className="rounded-xl border border-dash-border bg-[#FAFBFC] p-4 md:p-5"
              data-testid="analytics-comparison"
            >
              <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-dash-text-subtle mb-3">
                ...
                {comparisonItems.map((item) => {
                  ...
                  return (
                    <div key={item.key} className="min-w-[120px]">
                      <p className="text-xs text-dash-text-muted">{t(`analyticsPage.comparison.${item.key}`)}</p>
                      <p
                        className={[
