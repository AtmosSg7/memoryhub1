...
import { PLAN_CATALOG, formatPlanPrice } from "@/constants/planConfig";
              ...
              >
                {highlight && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span
                      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-semibold text-white shadow-[0_1px_0_rgba(255,255,255,0.2)_inset,0_6px_16px_-4px_rgba(79,70,229,0.5)]"
                ...
                <h3
                  className={`font-display text-[24px] font-bold tracking-[-0.025em] ${highlight ? "text-white" : "text-[#0A0A0B]"}`}
                >
                  {copy?.name}
                </h3>
                <p
                  className={`mt-2 text-[13.5px] leading-[1.55] ${highlight ? "text-white/65" : "text-[#52535E]"}`}
                >
                  {copy?.desc}
                </p>
                <div className="mt-7 flex items-baseline gap-1">
                  <span
                    className={`font-display text-[52px] font-black tracking-[-0.03em] leading-none ${highlight ? "text-white" : "text-[#0A0A0B]"}`}
                  >
                    {price}€
                  </span>
                  <span className={`text-[13.5px] ${highlight ? "text-white/55" : "text-[#8A8F98]"}`}>
                    {t("pricing.per")}
                  </span>
                </div>
                <button
                ...
                <ul className="mt-8 space-y-3.5">
                  {features.map((f) => (
                    <li
                      key={f}
                      className={`flex items-start gap-3 text-[14px] leading-[1.5] ${highlight ? "text-white/85" : "text-[#0A0A0B]/85"}`}
                    >
                      <span
                        className={`flex-shrink-0 mt-0.5 w-4 h-4 rounded-full flex items-center justify-center ${highlight ? "bg-white/10" : "bg-[#EEF2FF]"}`}
                      >
                        <Check
                          className={`w-3 h-3 ${highlight ? "text-[#0EA5E9]" : "text-[#4F46E5]"}`}
                          strokeWidth={3}
                        />
                      </span>
                      <span>{f}</span>
