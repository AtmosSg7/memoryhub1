...
export const CLIENT_PANEL_CLASS = "bg-dash-surface border border-dash-border rounded-xl p-5 md:p-6";

export const CLIENT_LIST_ROW_CLASS =
  "py-3 flex items-center justify-between gap-3 text-sm cursor-pointer hover:bg-dash-surface-muted -mx-2 px-2 rounded-lg transition-colors";
...
export const CLIENT_NOTE_CARD_CLASS =
  "w-full text-left rounded-lg border border-dash-border bg-dash-surface-muted px-3 py-2.5 hover:border-dash-primary/20 transition-colors";

export const CLIENT_FILTER_WRAP_CLASS = "mb-4";

export const CLIENT_SECTION_ACTION_CLASS = "gap-1.5";

export const CLIENT_DIVIDER_LIST_CLASS = "divide-y divide-dash-border-soft";
...
export function ClientSectionLink({ children, testId, ...props }) {
  return (
    <button
      type="button"
      data-testid={testId}
      className="mt-4 text-xs font-medium text-dash-primary hover:underline transition-colors"
      {...props}
    >
      {children}
    </button>
